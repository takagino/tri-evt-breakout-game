...
export default function useGameLogic() {
      ...
      e.pairs.forEach((pair) => {
        ...
        if (ball && paddle && !gameData.current.isHoldingBall) {
          if (ball.velocity.y > 0) {
            // パドルの法線ベクトル（画面上での「上」方向）を計算
            const nx = Math.sin(paddle.angle);
            const ny = -Math.cos(paddle.angle);

            // 現在の速度
            const vx = ball.velocity.x;
            const vy = ball.velocity.y;

            // 内積を計算して法線に対する反射ベクトルを求める
            const dot = vx * nx + vy * ny;
            let newVx = vx - 2 * dot * nx;
            let newVy = vy - 2 * dot * ny;

            // バーを振り上げた勢い（paddleVy は上移動時にマイナス）を法線方向に加算
            if (gameData.current.paddleVy < -1) {
              const swingForce = Math.abs(gameData.current.paddleVy) * 0.5;
              newVx += nx * swingForce;
              newVy += ny * swingForce;
            }

            // ボールが必ず上方向（マイナスY）に向かうように最低速度を保証
            if (newVy > -2) {
              newVy = -2;
            }
            ...
            // 跳ね返すたびにスピードを0.1ずつ増加
            gameData.current.currentBallSpeed += 0.1;
          }
        }
      });
    });
  ...
  const gameLoop = () => {
    ...
    const activeBallSpeed = state.currentBallSpeed;
    ...
    if (state.ball && state.gameState === 'playing') {
      ...
      if (state.ball.position.y > ch + 50) {
        ...
        if (state.lives <= 0) {
          ...
          state.ball = null;
          state.isHoldingBall = true;
          state.currentBallSpeed = BALL_SPEED; // ミスしたらスピードを元に戻す
        }
      }
